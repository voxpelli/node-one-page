...
const renderPage = function (page, currentPage, title, pageTemplate) {
  var config = page.config;

  var wrappers = (currentPage === 0 ? ['front-wrapper', 'page', 'layout'] : undefined);
  var frontData = {};
  var result;
  ...
  _.each(page.contentTypes, function (contentType) {
    if (activeContentType(pageTemplate.globalData, contentType)) {
      frontData[contentType.getId()] = contentType.getData(currentPage);
    }
  });
