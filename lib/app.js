...
VTOnePage.prototype._themeSetup = function (theme) {
  ...
  theme = theme || VTOnePage.basetheme;
...
VTOnePage.prototype._contentTypeSetup = function () {
  this.addContentType('speakers', './admin/speakers', 'speakers');
  this.addContentType('agenda', './admin/agenda', 'agenda');
  this.addContentType('puffIntro', './admin/puff', 'puffs', {
    name: 'Intro',
    key : 'puff-intro'
  });
  this.addContentType('puffDescription', './admin/puff', 'puffs', {
    name: 'Beskrivning',
    key : 'puff-description'
  });
  this.addContentType('puffRegistration', './admin/puff', 'puffs', {
  ...
  this.addContentType('stream1', './admin/stream', 'video', { key : 'stream1' });
  this.addContentType('stream2', './admin/stream', 'video', { key : 'stream2' });
  this.addContentType('sirtrevor', './admin/sirtrevor', 'blocks');
  this.addContentType('dictionary', './admin/dictionary', 'dictionary', { name: 'Benämningar' });
  this.addContentType(
    'images',
    './admin/images',
    _.size(this.config.admin.images) ? 'image' : false,
    {
      name: 'Bilder',
  ...
  this.addContentType('extracodeCSS', './admin/extracode', 'customcss', {
    name: 'Extra CSS',
    key : 'extracode-css',
    help : 'Ibland kan en specifik sajt behöva en liten knuff i rätt riktning stilmässigt. Detta är knuffen.'
  });
  this.addContentType('accounts', './admin/accounts', 'accountadmin');
};
...
VTOnePage.prototype.addContentType = function (name, module, requireFeature, options) {
  if (_.isObject(requireFeature)) {
    return this.addContentType(name, module, undefined, requireFeature);
  }
