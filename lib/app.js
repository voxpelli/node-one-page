...
onePagePrototype._defineTemplatePuffs = function () {
  const themeOptions = this.getThemeOptions();
  _.each(themeOptions.vtOnePageTemplates || {}, (data, template) => {
    _.each(data.puffs || [], (puffDefinition, name) => {
      puffDefinition.key = [template, puffDefinition.key].join('-');
      puffDefinition.multipage = true;
      puffDefinition.template = template;
      this.addContentType('./admin/puff', 'puffs', puffDefinition);
    });
  });
};
...
onePagePrototype.getStaticFiles = function () {
  return require('./utils/find-static-files')(this.themeEngine.theme);
};
...
onePageStatic.cliExport = function (options) {
  options = Object.assign({ externalDomains: ['cloudinary.com'] }, options || {});
  require('./cli-export')(this, options);
};
