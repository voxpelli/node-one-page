...
ContentType.prototype.setMainPageAssemble = function (assembleMainPage) {
  this.assembleMainPage = assembleMainPage;
  return this;
};

ContentType.prototype.addToApp = function (app) {
...
ContentType.prototype.getListTemplate = function () {
...
ContentType.prototype.handlePost = function (req, res, next) {
  ...
  complete = function (fields) {
    ...
    form.handle(fields, {
      success: that.doPost.bind(that, function (err) {
        if (err) {
          if (err.form) {
            var params = {};
            err.form.handlingError = err.message;
            params[that.formName] = err.form;
            that.assembleMainPage(req, res, next, params);
          } else {
            next(err);
          }
        } else {
          res.redirect('/admin' + (that.hash || ''));
        }
      }),
      error: function (form) {
        var params = {};
  ...
  this.doGet(this.getPath === true ? req.params.id : req.params)
    .then(function (content) {
      if (!content) {
        ...
        this.assembleMainPage(req, res, next, params);
      }
    }.bind(this))
    .then(undefined, next);
};
