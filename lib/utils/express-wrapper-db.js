...
module.exports = function (ExpressWrapper, ExpressConfig) {
  ...
  protoProps.constructor = function () {
    ...
    this.knex = require('knex')({ client: 'pg', connection: this.config.db });
  ...
  staticProps.knexConfig = function (env) {
    var db = {
      client: 'pg',
      connection: this.ExpressConfig.getConfig().db,
      migrations: {
        dependencies: {
          // 'prefix-of-child': 'foo',
        },
  ...
  staticProps.runMigrationTask = function (command, returnPromise) {
    ...
    var env = this.ExpressConfig.getConfig().env;
  ...
  staticProps.ExpressConfig = ExpressConfig.extend({}, {
