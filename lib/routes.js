  ...
  routes.index = function (req, res, next) {
    Promise.all([
      ...
      var speakers = result[0]
        , agenda = result[1]
        , varsByKey = result[2]
        , dictionary  = varsByKey.dictionary || {}
        , jsDictionary = {};

      //TODO: Extract to separate file
      _.each(config.admin.dictionary, function (value, key) {
        dictionary[key] = dictionary[key] || value[1];
        if (value[3] === true && (!value[2] || config.features[value[2]])) {
          jsDictionary[key] = dictionary[key];
        }
      });
      ...
      if (cache.activeFeatures.puffs && cache.vars['puff-registration'] && cache.vars['puff-registration'].body) {
        children.push({
          ...
          variables : {
            menuName : 'Anmäl',
            name : 'register',
            header : cache.vars['puff-registration'].header || 'Anmäl dig till Valtech Day',
